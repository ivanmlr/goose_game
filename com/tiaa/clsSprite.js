class clsSprite{
    constructor(pCtx, pId='', pImgPath, pNum_Frames, pFileExtension, pXoff, pYoff, pScale=1,pAlpha = 1 ){
        this.ctx=pCtx;
        this.Id= pId;
...
//----------------------------------------------------------------------------------
    LoadImages(){
        for (var i=1; i<=this.num_frames;i++){
            var tI=new clsImage(this._imgPath+"frame_"+i+this.pFileExtension, this.ctx, 0, this.Xoff, this.Yoff, this.scale, 1, this.alpha)
            tI.caption=this.Id;
            this.frames.push(tI);
